...
import { Building, Mail, Lock, Eye, EyeOff, ArrowLeft, Zap, User } from 'lucide-react';

const LoginFloorwing = ({ onLogin }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const success = onLogin(formData.email, formData.password, 'floorwing', formData.username);
    
    if (success) {
      navigate('/dashboard-floorwing');
    } else {
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-indigo-50 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        ...
        >
          ...
          <h2 className="text-3xl font-bold text-gray-900 mb-2">Floorwing Portal</h2>
          <p className="text-gray-600">Oversee floor activities and competitions</p>
        </motion.div>
        ...
        >
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <motion.button
              ...
              type="submit"
              disabled={isLoading}
              className="w-full flex justify-center items-center py-3 px-4 border border-transparent rounded-xl shadow-sm text-white bg-gradient-to-r from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 font-medium transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isLoading ? (
                <motion.div
                  animate={{ rotate: 360 }}
        ...
        <motion.div
          ...
          className="text-center"
        >
          <p className="text-gray-600 mb-4">Not a floorwing?</p>
